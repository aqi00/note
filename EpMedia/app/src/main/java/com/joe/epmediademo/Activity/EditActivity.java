...
import static com.joe.epmediademo.Utils.FileUtil.copyToPublicDownload;
...
import com.joe.epmediademo.Utils.FileUtil;
...
public class EditActivity extends AppCompatActivity implements View.OnClickListener {
	...
	private EditText et_clip_start, et_clip_end, et_crop_x, et_crop_y, et_crop_w, et_crop_h, et_rotation, et_text_x, et_text_y, et_text, et_textsize;
	...
	private void initView() {
		...
		et_textsize = (EditText) findViewById(R.id.et_textsize);
	...
	private void execVideo(){
		final String ttfPath = FileUtil.getSavePath() + "msyh.ttf";
		if(videoUrl != null && !"".equals(videoUrl)){
			...
			if(cb_text.isChecked())
				epVideo.addText(Integer.parseInt(et_text_x.getText().toString().trim()),Integer.parseInt(et_text_y.getText().toString().trim()),Integer.parseInt(et_textsize.getText().toString().trim()),"red",ttfPath,et_text.getText().toString().trim());
			mProgressDialog.setProgress(0);
			mProgressDialog.show();
			final String outPath = FileUtil.getSavePath() + "out.mp4";
			Log.d(TAG, "outPath: "+outPath);
			EpEditor.exec(epVideo, new EpEditor.OutputOption(outPath), new OnEditorListener() {
				@Override
				public void onSuccess() {
					runOnUiThread(() -> {
						Toast.makeText(EditActivity.this, "编辑完成:"+outPath, Toast.LENGTH_LONG).show();
						mProgressDialog.dismiss();
					});

					Uri dest = copyToPublicDownload(EditActivity.this, new File(outPath));
					if (dest != null) {
						Intent v = new Intent(Intent.ACTION_VIEW);
						v.setDataAndType(dest, "video/mp4");
						startActivity(v);
					}
				}
